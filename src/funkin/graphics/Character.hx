...
class Character extends FlxSprite
{
	...
	override function update(elapsed:Float)
	{
		...
		if (Conductor != null)
		{
			if (!isPlayer && holdTimer >= Conductor.stepCrochet * (0.0011 #if FLX_PITCH / (1) #end) * singDuration)
			{
				dance();
				holdTimer = 0;
			}
	...
	{
		if (hasAnimation(singAnimations[note.data.data % singAnimations.length]) && playAnim)
			this.playAnim(singAnimations[note.data.data % singAnimations.length], true);
        if(note.noteType.toLowerCase() == 'alt note' && hasAnimation(singAnimations[note.data.data] + '-alt')  && playAnim)
            this.playAnim(singAnimations[note.data.data % singAnimations.length] + '-alt', true);
